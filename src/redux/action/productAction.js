...
export const getProduct = (keyword = '', price = [10, 25000], category,currentPage=1) => async (dispatch) => {
    try {
        ...
        let link = `http://localhost:5000/api/v1/products?keyword=${keyword}&price[gte]=${price[0]}&price[lte]=${price[1]}&page=${currentPage}`;

        if (category) {
            link = `http://localhost:5000/api/v1/products?keyword=${keyword}&category=${category}&price[gte]=${price[0]}&price[lte]=${price[1]}&page=${currentPage}`;
        }
